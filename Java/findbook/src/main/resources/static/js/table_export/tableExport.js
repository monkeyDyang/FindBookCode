    ...
    tableExport: function (options) {
      ...
      if ( defaults.type == 'csv' || defaults.type == 'tsv' || defaults.type == 'txt' ) {
        ...
        $(el).filter(function () {
          ...
          //$hrows = $table.find('thead').first().find('div');
          docData    += '<Table>';
        ...
        $(el).filter(function () {
          ...
          // Header
          docData += '<table><thead>';
          //$hrows = $table.find('thead').find(defaults.theadSelector);
          $hrows = $('#gview_' + $table.attr('id')).find('thead').find(defaults.theadSelector);//此处修改，展示表头，原有写法与实际标签id存在出入20180528
          $hrows.each(function () {
            trData = "";
            ForEachVisibleCell(this, 'th,td', rowIndex, $hrows.length,
              function (cell, row, col) {
      ...
      */
      ...
      function GetColumnNames(table) {
          var result = [];
          $("#gview_" + $(table).attr('id')).find('table thead').find('th').each(function (index, el) {               
              //if ( $(el).attr("data-field") !== undefined )
              //  result[index] = $(el).attr("data-field");
              //else
              //  result[index] = index.toString();

              result[index] = $("#" + $(el).attr("id")).text();
          });
          return result;
      }
