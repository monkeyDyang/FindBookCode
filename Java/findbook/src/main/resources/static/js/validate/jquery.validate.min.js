...
$(function () {
    ...
    // 联系电话(手机/电话皆可)验证扩展    
    jQuery.validator.addMethod("isTelEx", function (value, element) {
        var tel = /(^[0-9]{3,4}\-[0-9]{3,8}$)|(^[0-9]{3,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}13[0-9]{9}$)|(^0{0,1}14[0-9]{9}$)|(^0{0,1}15[0-9]{9}$)|(^0{0,1}17[0-9]{9}$)|(^0{0,1}18[0-9]{9}$)/;;
        return this.optional(element) || tel.test(value) ;
    }, "请正确填写您的联系方式");
    // 匹配qq      
    ...
    // 只能输入两位正小数
    jQuery.validator.addMethod("isPosNumber", function (value, element) {
        return this.optional(element) || /^(([1-9][0-9])|(([0].\d{0,2}|[1-9][0-9].\d{0,2})))$/.test(value);
    }, "只能输入两位正小数");
    // 数值不包含负数(N位小数)
    jQuery.validator.addMethod("isNumePoint", function (value, element, param) {
        return this.optional(element) || (eval("/^(([1-9]\\d*)|\\d)(\\.\\d{1," + parseInt(param[0]) + "})?$/")).test(value);
        //}, "请输入正确的数值(可包含{0}位小数)");
    }, "只能输入包含{0}位小数的正数");
    //小时分钟    
    jQuery.validator.addMethod("isHourMin", function (value, element) {
        var strtemp = /^([0-1]{1}\d|2[0-3]):([0-5]\d)$/;
        return this.optional(element) || (strtemp.test(value));
    }, "请正确填写时间。");
});