...
class SpidermanPipeline(object):
    ...
    def Get_BaseInfo(self,item):
        ...
        # print(item)
        sql = 'select No from fb_book_baseinfo where Name like %s ' % ("'%"+item['Name']+"%'")
        cursor.execute(sql)
    ...
    def Get_URL(self,item):
        sql = 'select No from fb_book_url where Book_No = %s and Code = %s' % (item['Book_No'],item['Code'])
        cursor.execute(sql) 
        return cursor.fetchone()
    ...
    def process_item(self, item, spider):
        ...
        # BookURL
            ...
            if not exist:
                ...
                no = exist['No']
                # self.Update_URL(item,no) #如果存在则 不做处理
                print('Update_Success!!!!!!!')
        # Test
